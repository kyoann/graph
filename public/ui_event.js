...
function event_nodeCreated(createdNode) {
    ...
    if (!eventCallbackLinkTable.nodeCreated) {
        return;
    }
    for (var i = 0; i < callbacks.length; i++) {
...
function event_eventOccured(eventName, callbacksArgument) {
    console.log(eventName + " happened")
    var callbacks = eventCallbackLinkTable[eventName];
    if (!callbacks) {
        return;
    }
    for (var i = 0; i < callbacks.length; i++) {
        console.log(eventName + "  callback:");
        callbacks[i](callbacksArgument);
    }
}