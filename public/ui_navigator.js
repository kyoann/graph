function navigator_append(container, navigatorId, nodesModels, createDblClickCB) {
    ...
    navigatorDiv.addEventListener('dragover', function (event) {
        event.preventDefault();
    }, false);
    navigatorDiv.addEventListener('drop', function (event) {
        navigator_dropNodeOnNavigator(event);
    }, false);

    event_addModelEventListener("nodeDnDFrom" + navigatorId, navigator_nodeDndFromNavigator);

    navigator_init(navigatorDiv, nodesModels, createDblClickCB);
}

function navigator_dropNodeOnNavigator(ev) {
    ev.stopPropagation();
    ev.preventDefault();
    var nodeViewId = ev.dataTransfer.getData('Text');
    navigator_unlink(nodeViewId);
}

function navigator_nodeDndFromNavigator(nodeViewId) {
    navigator_unlink(nodeViewId);
}

function navigator_unlink(nodeViewId) {
    var nodeView = document.querySelector('#' + nodeViewId);
    var nodeId = nodeView.dataset.nodeid;
    var nodeViewParentId = nodeView.dataset.parentNodeViewId;
    if (!nodeViewParentId) {
        return;
    }
    var nodeParentView = document.querySelector('#' + nodeViewParentId);
    var nodeParentId = nodeParentView.dataset.nodeid;

    if (nodeParentId == undefined) {
        return false;
    }

    unlinkNode(nodeParentId, nodeId, function (updatedNode, unlinkedNodeId) {
        event_nodeUnlinked(updatedNode, unlinkedNodeId);
    });
}

function navigator_selected(navigatorDiv, nodeDiv, eraseColumnsAfter, createDblClickCB) {
    getNeighbours(nodeDiv.dataset.nodeid, function (neighbours) {
        ...
        var nodesViews = node_addNodes(nextColumnDiv, neighbours, navigatorDiv.id);
        for (var i = 0; i < nodesViews.length; i++) {
            nodesViews[i].addEventListener('click', (function (nodeView) {
                return function (e) {
                    e.stopPropagation();
                    navigator_selected(navigatorDiv, nodeView, true, createDblClickCB);
                };
            })(nodesViews[i]));
			
			if (createDblClickCB) {
				nodesViews[i].addEventListener('dblclick',function(nodeView) { return createDblClickCB(nodeView);}(nodesViews[i]));
			}
...
function navigator_init(navigatorDiv, nodesModels, createDblClickCB) {
    ...
    for (var i = 0; i < nodeDivs.length; i++) {
        ...
        nodeDiv.addEventListener('click', navigator_createNodeSelectedCB(navigatorDiv, nodeDiv,createDblClickCB));
        if (createDblClickCB) {
...
function navigator_createNodeSelectedCB(navigatorDiv, nodeDiv,createDblClickCB) {
    return function (e) {
        e.stopPropagation();
        navigator_selected(navigatorDiv, nodeDiv, true,createDblClickCB);
    };
}
