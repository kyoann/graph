function navigator_append(container,nodeModel) {
	var navigatorDiv = document.createElement('div');
	navigator.className = 'UI';
...
function navigator_selected(navigatorDiv,nodeDiv,done) {
	getNeighbours(nodeDiv.dataset.nodeid,function(neighbours){
		...
		for(var i = 0 ; i < nodesViews.length ; i++) {
			nodesViews[i].addEventListener('click',(function(nodeView) {return function() {navigator_selected(navigatorDiv,nodeView);};})(nodesViews[i]) );
		}
...
function navigator_init(navigatorDiv,nodeModel) {
	...
	nodeDiv.addEventListener('click',function() {navigator_selected(navigatorDiv,nodeDiv)});
}
		...
		for(var i = 0 ; i < colsList.length ; i++) {
			...
			if(selectedNode && selectedNode.dataset.nodeid == updatedNode.id) {
				...
				if(selectedNodeId2 != undefined) {
					node_highLightNodeDiv(getNodeView(colsList.item(i+1),selectedNodeId2).firstChild);
				}
			}
		}
	}
